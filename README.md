...
- 💬 **情绪语录与互动**：点击、长按、喂食和定时事件都会触发不同回应，并支持托盘中的“成人语录模式 / 普通语录模式”；高强度语录仅在成人模式出现，审核后的普通专属和双模式共用语录分别按分类生效。
- 🗂️ **语录库管理**：从托盘/菜单栏打开“管理语录库”，用表格新增、编辑、停用、删除语录，并随时调整它属于普通专属、双模式共用还是成人专属。
...
宠物的位置、大小、形象、喂食时间、鼠标穿透、置顶、语录模式、开机自启和快捷键会自动保存。语录库编辑器的内容也会单独保存，且保存后立即应用。
...
语录库文件与配置文件位于同一数据目录：`quotes_user.json`。删除语录前建议优先使用“停用/启用”，这样可以保留编号并随时恢复；“恢复内置审核结果”会清除用户对语录库的新增、修改、停用和删除结果。
